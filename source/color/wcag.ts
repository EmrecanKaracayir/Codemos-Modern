...
export function getContrastRatioHex6(hex6: string, refHex6: string): number {
...
export function getLumCompRgb(source: RGB, backdrop: RGB, alpha: number): RGB {
  normalizeRgb(source);
...
export function getColCompRgb(source: RGB, backdrop: RGB, alpha: number): RGB {
  normalizeRgb(source);
  normalizeRgb(backdrop);
...
function getContrastRgb(fgRgb: RGB, bgRgb: RGB): number {
  normalizeRgb(fgRgb);
  normalizeRgb(bgRgb);
...
function normalizeRgb(rgb: RGB): void {
  rgb[0] = rgb[0] / 255;
  rgb[1] = rgb[1] / 255;
  rgb[2] = rgb[2] / 255;
}

function denormalizeRgb(rgb: RGB): void {
  rgb[0] = rgb[0] * 255;
  rgb[1] = rgb[1] * 255;
  rgb[2] = rgb[2] * 255;
}

function getLumRgb(rgb: RGB): number {
...
function getLumBlendRgb(rgb: RGB): number {
  return rgb[0] * 0.3 + rgb[1] * 0.59 + rgb[2] * 0.11;
}

function clipColRgb(color: RGB): void {
...
function setLumRgb(color: RGB, lum: number): RGB {
...
function getAlphaCompRgb(opaque: RGB, transparent: RGB, alpha: number): RGB {
  return [
