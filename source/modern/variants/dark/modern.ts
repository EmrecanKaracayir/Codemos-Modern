...
export const LUM_SOURCE_ALPHA: number = 0.96;
...
export const getStyles = (variantConfig: VariantConfig): Styles => {
  ...
  const surfaceColor = getMixedColorHex7(palette.hic, 2, baseColor);
  const layerColor = getMixedColorHex7(palette.hic, 2, surfaceColor);
  ...
  return {
    ...
    fill: {
      ...
      tab: {
        activeFocused: `${flyoutColor}${getHexAlpha(100)}`,
        activeUnfocused: `${layerColor}${getHexAlpha(100)}`,
        inactiveFocused: `${surfaceColor}${getHexAlpha(100)}`,
    ...
    bg: {
      solid: {
        ...
        surface: `${surfaceColor}${getHexAlpha(100)}`,
        layer: `${layerColor}${getHexAlpha(100)}`,
